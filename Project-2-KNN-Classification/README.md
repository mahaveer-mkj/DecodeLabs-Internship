...
| **Process** | Train KNN, tune K using the Elbow Method | Avoids overfitting (K=1) and underfitting (K=100) |
| **Output** | Evaluate with Confusion Matrix, Precision, Recall, F1 | F1 Score used — not just accuracy |
...
> The green line marks the optimal K where error rate bottoms out. On the clean Iris benchmark, K=1 achieves zero error — demonstrating the effectiveness of proper feature scaling.
...
- Display the Elbow Method plot (saved as `elbow_plot.png`)
- Display the Confusion Matrix (saved as `confusion_matrix.png`)
- Print the full classification report with F1 scores
...
├── elbow_plot.png            # Output: Elbow Method chart
├── confusion_matrix.png      # Output: Confusion Matrix heatmap
└── README.md                 # This file
```
...
│                   PROCESS STAGE                             │
│                                                             │
│  find_optimal_k()  →  Elbow Method (K=1 to K=30)           │
│                              ↓                              │
...
run_pipeline(
    ...
    k_max        = 30,     # Change the upper bound for K search
)
```
...
Choosing K manually is guesswork. The Elbow Method iterates K from 1 to `k_max`, records the test error at each step, and identifies the point of diminishing returns:
...
| `pandas` | 2.0+ | Data handling and reporting |
